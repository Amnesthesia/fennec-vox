...
export async function extractChapters(
  ...
  const title  = opfDoc('dc\\:title,  dcterms\\:title').first().text().trim()   || 'Unknown Title';
  const author = opfDoc('dc\\:creator, dcterms\\:creator').first().text().trim() || 'Unknown Author';
  ...
  for (let spineIdx = 0; spineIdx < spineItems.length; spineIdx++) {
    ...
    const chapterTitle  = doc('h1, h2').first().text().trim() || `Chapter ${chapters.length + 1}`;
    chapters.push({ index: chapters.length, spineIndex: spineIdx, id: idref, title: chapterTitle, text });
  }
