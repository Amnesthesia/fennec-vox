...
function makeChapter(index: number, text = 'Sample text for chapter.'): Chapter {
  return { index, spineIndex: index, id: `ch-${index}`, title: `Chapter ${index}`, text };
}
...
function makeIO(overrides: Partial<ConversionIO> = {}): ConversionIO {
  ...
  return {
    ...
    saveChapterAudio: vi.fn().mockImplementation(async (idx: number, data: Buffer, fmt: string) => {
...
function makeFullOpenai(audioBuf = Buffer.from('audio')) {
  return {
    chat: {
      completions: {
        create: vi.fn().mockResolvedValue({ choices: [{ message: { content: 'marked text' } }] }),
      },
    },
    audio: {
      speech: {
...
describe('processChapter', () => {
  ...
  it('runs markup → TTS → saves audio, emits progress events', async () => {
    ...
    const result = await processChapter(chapter, {
      provider: 'claude-haiku', anthropic, openai,
      voice: 'alloy', format: 'mp3', ttsModel: 'tts-1',
      chunkSize: 2000, concurrency: 1, total: 1, io,
    });
    ...
    const progressTypes = (io.onProgress as ReturnType<typeof vi.fn>).mock.calls.map(c => c[0].type);
  ...
  it('uses markup cache when available, skips API call', async () => {
    const chapter   = makeChapter(0);
    const io        = makeIO({ getMarkupCache: vi.fn().mockResolvedValue('cached markup') });
...
describe('runConversion', () => {
  ...
  function makeOpts(chapters: Chapter[], overrides: Partial<RunConversionOpts> = {}): RunConversionOpts {
    return {
      chapters,
      completedChapters: {},
  ...
  it('processes all pending chapters and assembles audio', async () => {
    const chapters = [makeChapter(0, 'Chapter zero text.'), makeChapter(1, 'Chapter one text.')];
  ...
  it('skips already-completed chapters', async () => {
    ...
    const audioStore  = new Map<string, Buffer>([['ch-0.mp3', Buffer.from('pre')]]);
    const io = makeIO({
      saveChapterAudio: vi.fn().mockImplementation(async (idx: number, data: Buffer, fmt: string) => {
        const key = `ch-${idx}.${fmt}`;
        audioStore.set(key, data);
      ...
      readChapterAudio: vi.fn().mockImplementation(async (key: string) => audioStore.get(key) ?? Buffer.from('pre')),
    });

    const completedChapters = {
      0: { title: 'Ch 0', file: 'ch-0.mp3', chars: 5, audioBytes: 3, completedAt: '' },
    };

    const result = await runConversion(makeOpts(chapters, { completedChapters, io }));
  ...
  it('respects cancellation between chapters', async () => {
    ...
    const chapters = [makeChapter(0), makeChapter(1)];
    await expect(runConversion(makeOpts(chapters, { io }))).rejects.toThrow('Conversion cancelled');
  });
  ...
  it('emits assembly progress event', async () => {
    ...
    const types = (io.onProgress as ReturnType<typeof vi.fn>).mock.calls.map(c => c[0].type);
