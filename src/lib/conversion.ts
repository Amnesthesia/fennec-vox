...
import { splitIntoChunks, padded } from './text';
import { pLimit } from './pLimit';
import type {
  Chapter, ChapterRecord, MarkupProvider,
  TtsVoice, TtsFormat, TtsModel, ProgressEvent,
} from './types';
...
export interface ConversionIO {
  ...
  saveChapterAudio(chapterIndex: number, data: Buffer, format: TtsFormat): Promise<string>;
  ...
  onAssemble?: (chapterKeys: string[], chapterTitles: string[], bookTitle: string, bookAuthor: string) => Promise<Buffer>;
}
...
export async function processChapter(
  ...
  const { provider, anthropic, openai, voice, format, ttsModel, chunkSize, concurrency, total, ttsInstructions, io } = opts;
  ...
  const audioBuffer = await synthesiseText(
    openai, ttsText, voice, innerFmt, ttsModel, concurrency,
    (i) => io.chunkKey(index, i, innerFmt),
    io.audioChunkCache,
    (i, tot, wasCached) => io.onLog(`  [${index}]       TTS chunk ${i + 1}/${tot}${wasCached ? ' (cached)' : ''}…`),
    ttsInstructions,
  );

  const file = await io.saveChapterAudio(index, audioBuffer, innerFmt);
  io.onLog(`  [${index}] Chapter ${index} done ✓ (${(audioBuffer.length / 1024).toFixed(0)} KB)`);
  io.onProgress({ type: 'chapter_done', index, title, file, audioBytes: audioBuffer.length });
...
export interface RunConversionOpts {
  ...
  bookTitle?:          string;
  bookAuthor?:         string;
  onChapterComplete?:  (chapter: Chapter, record: ChapterRecord) => Promise<void>;
}
...
export async function runConversion(opts: RunConversionOpts): Promise<RunConversionResult> {
  const { chapters, completedChapters, io, concurrency, onChapterComplete, format } = opts;

  const processorOpts: ProcessorOpts = {
    provider: opts.provider, anthropic: opts.anthropic, openai: opts.openai,
    voice: opts.voice, format: opts.format, ttsModel: opts.ttsModel,
    chunkSize: opts.chunkSize, concurrency: opts.concurrency,
    total: chapters.length, ttsInstructions: opts.ttsInstructions, io,
  };
  ...
  const tasks = pending.map(chapter => async () => {
    ...
    const record: ChapterRecord = {
      ...
      chars: chapter.text.length, audioBytes,
  ...
  const sortedKeys = Object.keys(completedChapters).map(Number).sort((a, b) => a - b);
  ...
  if ((format === 'm4b' || format === 'm4a') && io.onAssemble) {
    ...
    assembled = await io.onAssemble(chapterKeys, chapterTitles, opts.bookTitle ?? '', opts.bookAuthor ?? '');
  } else {
    const parts = await Promise.all(sortedKeys.map(k => io.readChapterAudio(completedChapters[k]!.file)));
