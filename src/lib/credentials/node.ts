...
export async function getCredentials(): Promise<{ anthropicKey: string; openaiKey: string }> {
...
export async function saveCredentials(creds: Partial<{ anthropicKey: string; openaiKey: string }>): Promise<void> {
  const keytar = getKeytar();
  if (!keytar) return;
