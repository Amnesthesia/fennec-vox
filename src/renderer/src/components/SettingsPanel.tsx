      ...
      <div className="modal" role="dialog" aria-modal="true" aria-label="Settings">
        <div className="modal-inner">
          ...
          <p className="modal-sub">
            ...
            Claude Haiku is used for SSML markup when an Anthropic key is present;
            otherwise GPT-4o mini is used.
          </p>

          {/* OpenAI */}
          <div className="settings-key-label">
            <span className="section-label" style={{ marginBottom: 0 }}>OpenAI (TTS + fallback markup)</span>
            <button
              className="settings-get-key-link"
              type="button"
              onClick={() => void window.api.openExternal('https://platform.openai.com/api-keys')}
            >
              Get key →
            </button>
          </div>
          <div className="key-row" style={{ marginTop: 6 }}>
            ...
            {openaiKey && (
              <button className="btn btn-secondary" style={{ fontSize: 11, padding: '5px 8px' }}
                onClick={() => setOpenaiKey('')} type="button" title="Clear key">✕</button>
            )}
          </div>
          {openaiKey && (
          ...
          <div className="settings-key-label" style={{ marginTop: 8 }}>
            <span className="section-label" style={{ marginBottom: 0 }}>Anthropic (Claude Haiku — optional)</span>
            <button
              className="settings-get-key-link"
              type="button"
              onClick={() => void window.api.openExternal('https://console.anthropic.com/settings/keys')}
            >
              Get key →
            </button>
          </div>
          <div className="key-row" style={{ marginTop: 6 }}>
            ...
            {anthropicKey && (
              <button className="btn btn-secondary" style={{ fontSize: 11, padding: '5px 8px' }}
                onClick={() => setAnthropicKey('')} type="button" title="Clear key">✕</button>
            )}
          </div>
          {anthropicKey && (
            <div style={{ fontSize: 10, color: 'var(--text-secondary)', marginBottom: 12 }}>
