...
const argv = yargs(hideBin(process.argv))
	...
	.example(
		"$0 book.pdf --concurrency 4 --yes",
		"PDF input, parallel, no prompt",
	)
	...
	.option("redo-tts", {
		...
		default: false,
		description:
			"Re-run TTS synthesis for all chapters (keeps narrator markup cache)",
	})
	.option("yes", {
		alias: "y",
		type: "boolean" as const,
	...
	.demandCommand(
		1,
		"Please supply an EPUB or PDF file path as the first argument.",
	)
	.help()
	.parseSync();
...
function getChapterHtml(book: EPub, id: string): Promise<string> {
	return new Promise((resolve, reject) => {
		book.getChapter(id, (error, text) => {
			if (error)
				reject(new Error(`Failed to read chapter "${id}": ${String(error)}`));
			else resolve(text ?? "");
		});
	});
}
...
export async function extractChapters(
	...
	for (let spineIdx = 0; spineIdx < book.flow.length; spineIdx++) {
		...
		try {
			...
			log(
				`  Warning: skipping spine[${spineIdx}] (${itemId}): ${(e as Error).message}`,
			);
			continue;
		}
		...
		log(
			`  [${String(chapters.length - 1).padStart(3)}] "${chapterTitle}" — ${text.length.toLocaleString()} chars`,
		);
	}
...
export async function extractChaptersPdf(
	...
	const author =
		(data.info?.Author as string | undefined)?.trim() || "Unknown Author";
	...
	chapters.forEach((ch) => {
		log(
			`  [${String(ch.index).padStart(3)}] "${ch.title}" — ${ch.text.length.toLocaleString()} chars`,
		);
	});
...
export async function extractContent(
	...
	throw new Error(
		`Unsupported file format "${ext}". Only .epub and .pdf are supported.`,
	);
}
		...
		chunks.map((chunk, i) => async () => {
			...
			for (let attempt = 1; attempt <= 3; attempt++) {
				try {
					...
					if (instructions)
						(params as unknown as Record<string, unknown>).instructions =
							instructions;
					...
					if (attempt < 3) {
						log(
							`  TTS chunk ${i + 1}/${chunks.length} failed (attempt ${attempt}), retrying…`,
						);
...
function displayCostEstimate(
	...
	const providerLabel =
		est.provider === "claude-haiku" ? "Claude Haiku" : "GPT-4o mini";
	console.log(`\n┌${rule}┐`);
	...
	console.log(
		row(
			"Chapters to process :",
			`${n(est.pendingChapters)}  (${n(est.skippedChapters)} already done)`,
		),
	);
	console.log(row("Total input chars   :", n(est.totalInputChars)));
	console.log(
		row("Concurrency         :", `${concurrency} chapter(s) in parallel`),
	);
	console.log(`├${rule}┤`);
	console.log(
		`│  ${providerLabel} (SSML markup)${" ".repeat(W - 20 - providerLabel.length)}│`,
	);
...
export async function processChapter(
	...
	const audioBuffer = await synthesiseText(
		...
		(i) =>
			path.join(
				chaptersDir,
				`chapter-${padded(index)}-chunk-${padded(i)}.${innerFormat}`,
			),
		(i, total, cached) =>
			log(
				`  [${index}]       TTS chunk ${i + 1}/${total}${cached ? " (cached)" : ""}…`,
			),
		ttsInstructions,
	);
	...
	const chapterFile = path.join(
		chaptersDir,
		`chapter-${padded(index)}.${innerFormat}`,
	);
	await fs.writeFile(chapterFile, audioBuffer);
	const kb = (audioBuffer.length / 1024).toFixed(0);
	log(
		`  [${index}]       Saved: ${path.relative(process.cwd(), chapterFile)} (${kb} KB)`,
	);
...
async function main(): Promise<void> {
	...
	const inputArg =
		argv._.find((p) => path.resolve(String(p)) !== selfPath) ?? argv._[0];
	...
	if (inputExt !== ".epub" && inputExt !== ".pdf") {
		logError(
			`Unsupported format "${inputExt}". Only .epub and .pdf are accepted.`,
		);
		process.exit(1);
	}
	...
	if (redoTts && !noResume) {
		log(
			"--redo-tts: clearing chapter audio and resetting progress (narrator markup cache kept)",
		);
	...
	if (estimate.pendingChapters === 0) {
		...
		const answer = await askConfirmation(
			"Proceed with these API calls? [y/N] ",
		);
	...
	const completedKeys = Object.keys(progress.completedChapters).sort(
		(a, b) => parseInt(a, 10) - parseInt(b, 10),
	);

	if (completedKeys.length === 0) {
		logError("No chapters completed.");
		process.exit(1);
	}
	...
	if (missing.length > 0)
		log(
			`Warning: ${missing.length} chapter(s) skipped: [${missing.join(", ")}]`,
		);
	...
	const outputFile = path.join(
		outputDir,
		`${path.basename(inputPath, inputExt)}.${format}`,
	);
	...
	if (format === "m4b" || format === "m4a") {
		...
		const parts = await Promise.all(
			completedKeys.map((k) =>
				fs.readFile(progress.completedChapters[Number(k)]?.file),
			),
		);
	...
	log(
		`Total size: ${totalMB.toFixed(2)} MB | Chapters: ${completedKeys.length}/${chapters.length}`,
	);
	emitProgress({
		type: "complete",
		outputFile,
		totalMB,
		chapters: completedKeys.length,
		total: chapters.length,
	});
}
