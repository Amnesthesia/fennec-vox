...
async function getApiKey(): Promise<string> {
  ...
  const keytar = (require('keytar') as typeof import('keytar')).default ?? require('keytar');
  const key = await keytar.getPassword(SERVICE, 'openai-api-key');
  if (!key) throw new Error('No OpenAI API key found in keychain. Set OPENAI_API_KEY env var or save a key in the app first.');
  return key;
}

interface Poem { slug: string; text: string }

async function loadPoems(): Promise<Poem[]> {
  const files = (await fs.readdir(POEMS_DIR)).filter(f => f.endsWith('.txt')).sort();
  return Promise.all(
    files.map(async f => ({
      slug: f.slice(0, -4),
...
async function generateOne(apiKey: string, voice: string, poem: Poem): Promise<void> {
  ...
  const res = await fetch('https://api.openai.com/v1/audio/speech', {
    ...
    body: JSON.stringify({ model: MODEL, voice, input: poem.text, response_format: 'mp3', instructions }),
  });
  ...
  console.log(`  done   ${voice}/${poem.slug}  (${(buf.length / 1024).toFixed(0)} KB)`);
}
...
async function main() {
  ...
  console.log(`Poems: ${poems.length}  |  Voices: ${VOICES.length}  |  Total: ${total} files  |  Concurrency: ${concurrency}`);
  if (force) console.log('  (--force: regenerating all files)');
  console.log('');

  // Build flat task list: all voice × poem combinations
  const tasks = VOICES.flatMap(voice => poems.map(poem => ({ voice, poem })));
    ...
    function dispatch(): void {
      while (active < concurrency && idx < tasks.length) {
        ...
        generateOne(apiKey, voice, poem)
          .catch(e => {
            console.error(`  ERROR  ${voice}/${poem.slug}: ${(e as Error).message}`);
            errors++;
          })
          .finally(() => {
            active--;
